...
use common::Result;
use wallet::types::{Slate, BlockFees};
use crate::api::error::ApiError;
use crate::api::router::{WalletContainer, trace_state_and_body, trace_create_response};
...
fn handle_receive_tx(state: &State, body: &Chunk) -> Result<Response<Body>> {
    ...
    Ok(trace_create_response(&state, StatusCode::OK, mime::APPLICATION_JSON, serde_json::to_string(&slate)?))
}
...
fn handle_build_coinbase(state: &State, body: &Chunk) -> Result<Response<Body>> {
    ...
    Ok(trace_create_response(&state, StatusCode::OK, mime::APPLICATION_JSON, serde_json::to_string(&cb_data)?))
}
